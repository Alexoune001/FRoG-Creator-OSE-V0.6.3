...
Dim Ending As String
    ...
    For i = 1 To 4
        ...
        If FileExiste(Rep_Theme & "\Jeu\magasin" & Ending) Then Me.Picture = LoadPNG(App.Path & Rep_Theme & "\Jeu\magasin" & Ending)
    Next i
...
If picFixItems.Tag >= 1 And picFixItems.Tag <= MAX_ITEMS Then
    For i = 1 To MAX_INV
        ...
        Else
            frmFixItem.cmbItem.AddItem "Slot inutiliser"
        End If
    Next i
    frmFixItem.cmbItem.ListIndex = 0
    If GetPlayerInvItemNum(MyIndex, 1) > 0 Then
        i = ((Item(GetPlayerInvItemNum(MyIndex, 1)).Data1 - GetPlayerInvItemDur(MyIndex, 1)) * (Item(GetPlayerInvItemNum(MyIndex, 1)).Data2 / 5)) \ 2
        If i <= 0 Then i = 1
        frmFixItem.Label2.Caption = "Prix : " & i & " " & Trim$(Item(picFixItems.Tag).name)
    Else
        frmFixItem.Label2.Caption = "Prix : ??? " & Trim$(Item(picFixItems.Tag).name)
    End If
